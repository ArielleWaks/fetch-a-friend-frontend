...
React single-page app for **Fetch a Friend**, a marketplace that connects pet owners with pet sitters. Users can browse and manage jobs, profiles, bookmarks, zip-based sitter search, and file uploads.
...
- **Running backend** — MySQL (e.g. Docker Compose in the backend repo) and `./gradlew bootRun` so the API is available on port **8080** before you rely on auth, jobs, or uploads.
...
- Calls like `/api/auth/signin`, `/api/jobs`, and `/file/...` reach Spring Boot on **8080**.
...
- `src/pages/` — screens (auth, jobs, profile, bookmarks, etc.)
